...
mod tests {
    ...
    fn test_resolve_char_code() {
        ...
        assert_eq!(
            resolve_char_code(Some("WinAnsiEncoding"), &diffs, 65),
            '\u{2022}'
        );
        // WinAnsi for 0x80
        assert_eq!(
            resolve_char_code(Some("WinAnsiEncoding"), &HashMap::new(), 0x80),
            '\u{20AC}'
        );
        // Plain ASCII
        assert_eq!(
            resolve_char_code(Some("WinAnsiEncoding"), &HashMap::new(), 0x41),
            'A'
        );
    }
}
