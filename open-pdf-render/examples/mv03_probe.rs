...
fn main() {
    ...
    println!(
        "replayer 1-tegel @0.3: {} ms ({}x{})",
        tref.elapsed().as_millis(),
        one.width,
        one.height
    );
    for tile_px in [512u32, 1024] {
        ...
        if diff > 0 {
                ...
                .enumerate()
            {
                ...
                if x % tb <= 1 || x % tb >= tb - 2 || y % tb <= 1 || y % tb >= tb - 2 {
                    on_boundary += 1;
                }
                for c in 0..4 {
            ...
            println!(
                "   diff-pixels: {}  op tegelrand: {}  max-kanaal-delta: {}  eerste: {:?}",
    ...
    // vergeleken met dezelfde uitsnede uit de 1-tegel-render.
    {
        ...
        for y in 0..512usize.min(one.height as usize) {
            for x in 0..500usize {
                ...
                let ti = (y * 500 + x) * 4;
                if one.rgba[fi..fi + 4] != with_cull.data()[ti..ti + 4] {
                    d_cull += 1;
                }
                if one.rgba[fi..fi + 4] != no_cull.data()[ti..ti + 4] {
                    d_nocull += 1;
                }
            }
        }
        println!(
            "halvering: diff-met-cull={}  diff-zonder-cull={}",
            d_cull, d_nocull
        );
        ...
        for (i, (a, b)) in one
            .rgba
            .chunks_exact(4)
            .zip(tiled512.rgba.chunks_exact(4))
            .enumerate()
        {
            let d: i32 = (0..4)
        ...
        if let Some(pm) = tiny_skia::Pixmap::from_vec(
            vis,
            tiny_skia::IntSize::from_wh(one.width, one.height).unwrap(),
        ) {
            let out = std::env::temp_dir().join("mv03_tile_diff.png");
            let _ = pm.save_png(&out);
            println!(
                "diff-vis: {}  worst delta {} @ ({},{})",
                out.display(),
                worst.0,
                worst.1,
                worst.2
    ...
    println!(
        "tiled @3.0 tegel=1024px: {} ms ({}x{})",
        tz.elapsed().as_millis(),
        z.width,
        z.height
    );
