...
async fn pool_render_matches_inproc_for_nkd1a_p4() {
    let exe =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/debug/pdfium-worker.exe");
    if !exe.exists() {
    ...
    // 1. Render via pool
    let workers = app_lib::worker_pool::spawn::spawn_pool(1, &exe)
        .await
        .expect("spawn pool");
    let pool = app_lib::worker_pool::WorkerPool::new(workers);
    assert!(pool.is_ready(), "pool not ready");

    let (pw, ph, prgba) = pool
        .render(path, page, scale, 0)
        .await
        .expect("pool render");

    // 2. Render in-proc. PDFium must be initialised first; idempotent.
    let dll_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/debug");
    app_lib::pdfium_renderer::init_pdfium(&dll_dir).expect("init pdfium");

    let bytes = std::fs::read(path).expect("read pdf");
    let cache = app_lib::pdfium_renderer::PdfiumDocCache::default();
    let handle =
        app_lib::pdfium_renderer::get_or_load_pdfium_doc_with_bytes(path, Arc::new(bytes), &cache)
            .expect("load pdf");
    let (iw, ih, irgba) =
        app_lib::pdfium_renderer::render_page_to_rgba(handle.document(), page, scale, 0)
    ...
    assert!(
        diff_pct < 0.1,
        "pool rgba differs from in-proc by {:.3}% ({} bytes)",
        diff_pct,
        diff
    );
    println!(
        "PARITY OK: {}x{}, diff={} bytes ({:.4}%)",
        pw, ph, diff, diff_pct
    );
}
