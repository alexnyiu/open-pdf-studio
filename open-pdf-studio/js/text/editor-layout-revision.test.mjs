...
test('layout revision changes for every paragraph, geometry, collision, and owner input', () => {
  ...
  const cases = [
    ...
    [() => { config.sourceWidth -= 1; }, () => { config.sourceWidth += 1; }],
    [() => { config.substitutionWidthAllowance = 0; }, () => { config.substitutionWidthAllowance = 1; }],
    [() => { config.effectiveContentWidth += 0.25; }, () => { config.effectiveContentWidth -= 0.25; }],
